...
export async function getBehandlinger(
  ...
  const response = await fetch(
    `http://localhost:8089/pen/springapi/psak2/behandling/feilende`,
    {
      headers: {
...
export async function getBehandling(
  ...
  const response = await fetch(
    `http://localhost:8089/pen/springapi/psak2/behandling/${behandlingId}`,
    {
      headers: {
        Authorization: `Bearer ${accessToken}`,
        'X-Request-ID': crypto.randomUUID(),
...
export async function fortsettBehandling(
  ...
  const response = await fetch(
    `http://localhost:8089/pen/springapi/psak2/behandling/${behandlingId}/fortsett`,
    {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${accessToken}`,
